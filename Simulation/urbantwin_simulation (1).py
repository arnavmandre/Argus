...
What changed vs. the first version
    * Buildings live inside zones. They are route endpoints, have their own
      heat / flood / crowding / accessibility numbers, and feed the headline
      metrics and the Advisor.
...
class Building:
    ...
    accessibility: float   # 0 = stairs only ... 1 = fully step-free
...
DEFAULT_CITY = {
    ...
    "buildings": [
        #         id     name                     type           zone      cap   cool  access flood
        Building("H1",   "Riverside Homes",       "residential", "Zone C",    0, 0.25, 0.50, 0.85),
        Building("H2",   "Old Town Apartments",   "residential", "Zone B",    0, 0.35, 0.65, 0.50),
        Building("H3",   "Garden Court",          "residential", "Zone A",    0, 0.60, 0.90, 0.15),
        Building("T1",   "Central Transit Hub",   "transit_hub", "Zone A", 3200, 0.40, 0.95, 0.35),
        Building("W1",   "Business Park Offices", "office",      "Zone A", 4500, 0.90, 0.90, 0.20),
        Building("S1",   "City College",          "school",      "Zone B", 3500, 0.55, 0.80, 0.40),
        Building("HOS1", "City General Hospital", "hospital",    "Zone B", 2400, 0.85, 0.95, 0.30),
        Building("M1",   "Market Square",         "market",      "Zone C", 5000, 0.15, 0.60, 0.80),
    ],
    "routes": [
        # Riverside Homes (Zone C) -> Central Transit Hub (Zone A): the original 4 corridors
        Route("R1",  "H1", "T1",   1.2, 0.20, 0.90, 0.95, 0.35, 1.00, ("Zone C", "Zone A")),
        Route("R2",  "H1", "T1",   1.5, 0.75, 0.60, 0.88, 0.25, 0.70, ("Zone C", "Zone B", "Zone A")),
        Route("R3",  "H1", "T1",   1.0, 0.10, 0.25, 0.55, 0.45, 0.35, ("Zone C",)),
        Route("R4",  "H1", "T1",   1.8, 0.85, 0.85, 0.92, 0.15, 0.40, ("Zone C", "Zone B", "Zone A")),
        # Riverside Homes -> Market Square (both Zone C)
        Route("R5",  "H1", "M1",   0.8, 0.15, 0.25, 0.55, 0.40, 0.10, ("Zone C",)),
        Route("R6",  "H1", "M1",   1.3, 0.60, 0.80, 0.90, 0.25, 0.20, ("Zone C",)),
        # Riverside Homes -> City General Hospital
        Route("R7",  "H1", "HOS1", 2.0, 0.20, 0.30, 0.60, 0.35, 0.30, ("Zone C", "Zone B")),
        Route("R8",  "H1", "HOS1", 2.6, 0.55, 0.85, 0.95, 0.20, 0.50, ("Zone C", "Zone A", "Zone B")),
        # Old Town Apartments (Zone B) -> City General Hospital
        Route("R9",  "H2", "HOS1", 0.9, 0.30, 0.60, 0.90, 0.40, 0.30, ("Zone B",)),
        Route("R10", "H2", "HOS1", 1.2, 0.80, 0.60, 0.85, 0.20, 0.20, ("Zone B",)),
        # Old Town Apartments -> City College
        Route("R11", "H2", "S1",   0.7, 0.25, 0.65, 0.88, 0.45, 0.45, ("Zone B",)),
        Route("R12", "H2", "S1",   1.0, 0.80, 0.60, 0.92, 0.20, 0.25, ("Zone B",)),
        # Old Town Apartments -> Business Park Offices
        Route("R13", "H2", "W1",   1.6, 0.15, 0.60, 0.80, 0.40, 0.60, ("Zone B", "Zone A")),
        Route("R14", "H2", "W1",   2.0, 0.60, 0.90, 0.95, 0.20, 0.55, ("Zone B", "Zone A")),
        # Garden Court (Zone A) -> Business Park Offices
        Route("R15", "H3", "W1",   0.8, 0.30, 0.90, 0.95, 0.35, 0.50, ("Zone A",)),
        Route("R16", "H3", "W1",   1.1, 0.80, 0.90, 0.92, 0.15, 0.30, ("Zone A",)),
        # Garden Court -> Central Transit Hub
        Route("R17", "H3", "T1",   0.6, 0.25, 0.90, 0.95, 0.50, 1.00, ("Zone A",)),
        Route("R18", "H3", "T1",   0.9, 0.70, 0.90, 0.95, 0.25, 0.70, ("Zone A",)),
    ],
}
...
_CITIZEN_UNIT_FIELDS = (
    ...
    "green_preference", "transit_preference", "accessibility_need",
)
_CITIZEN_REQUIRED = (
...
def load_citizens(path: Union[str, Path] = DEFAULT_CITIZENS_PATH) -> List[Citizen]:
    ...
    transit_preference, accessibility_need.  Optional: weight (default 1.0).
    ...
    for i, rec in enumerate(records):
        ...
        citizens.append(Citizen(
            ...
            accessibility_need=float(rec["accessibility_need"]),
            weight=float(weight),
        ))
...
def simulate(
    ...
    for z in city["zones"]:
        ...
        if interventions.get("accessibility_boost"):
            z.accessibility = clamp(z.accessibility + interventions["accessibility_boost"], 0.0, 1.0)
    ...
    for c in citizens:
        ...
        dest_access_gap = (1 - dest.accessibility) * 100
        ...
        personal_comfort = clamp(
            ...
            # Arrival end of the trip: crowded entrance / hard-to-enter destination.
            - 0.08 * dest_crowd * (1 - c.crowd_tolerance)
            - 0.10 * dest_access_gap * c.accessibility_need
        )
    ...
    zone_gap = 1 - mean(z.accessibility for z in city["zones"])
    building_gap = 1 - mean(b.accessibility for b in city["buildings"])
    access_gap = 0.6 * zone_gap + 0.4 * building_gap   # zones + step-free buildings
    ...
    for b in city["buildings"]:
        ...
        building_rows.append({
            ...
            "accessibility": round(b.accessibility * 100, 2),
...
def urban_advisor(result: Dict) -> Dict:
    ...
    if m["rain_impact"] >= 30:
        ...
        reasons.append(
            "Rainfall is creating meaningful accessibility and safety penalties"
...
def run_tests() -> None:
    ...
    ghost_home = Citizen("X1", "test", "NOPE", "T1", .5, .5, .5, 4.0, .5, .5, .5)
    expect_error(lambda: simulate(40, 80, 80, 100_000, citizens=[ghost_home]))
    no_route = Citizen("X2", "test", "H3", "HOS1", .5, .5, .5, 4.0, .5, .5, .5)   # exists, but no route
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        p.write_text(json.dumps({"citizens": [dict(
            ...
            transit_preference=.5, accessibility_need=.5)]}))
